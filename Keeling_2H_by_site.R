...
###note: the above used newer files for isotpes provided by Rich on 07/21/2021
...
layer.isotope <- function(hdf5_file, sitename, which_part = "H2") {
  ...
  ### which_part    : Either "H2" or "O18"
    ...
    iso_con <- "rtioMoleDryH2o" ### used the DryH2o
    if (which_part == "H2") {
...
for (j in 1:length(flist)) {
  ...
  keelings <- merged_dt %>% group_by(dom) %>% na.omit() %>% 
	    do(model = lm(I(H2_isoH2o*H2_rtioMoleDryH2o) ~ H2_rtioMoleDryH2o, data=.))
